...
Once reflectance data is written as a raster file, the [`spinR`](https://gitlab.com/jbferet/spinr) package includes a set of functions for the computation of various spectral indices. 
...
Indices <- spinR::ComputeSpectralIndices_Raster(Refl = Refl, SensorBands = SensorBands,
...
S2BandSelect <- list()
S2BandSelect$CHL <- S2BandSelect$lai <- S2BandSelect$EWT <- S2BandSelect$LMA <- c('B3','B4','B5','B6','B7','B8','B11','B12')
...
modelSVR <- train_prosail_inversion(minval = minval, maxval = maxval, Parms2Estimate = Parms2Estimate,
