...
property of interest based on a sensor reflectance LUT simulated with PROSAIL
...
url <- "https://gitlab.com/jbferet/myshareddata/-/raw/master/prosail_vignette_illustrations/Hybrid_Inversion/FlowChart_HybridInversion.png"
...
However, this requires users to define their own  `input_prosail` and `refl_lut` 
variables. 
...
The production of a surface reflectance LUT requires a variable `input_prosail` 
as input. 
When running  __4SAIL__, the expected list of parameters includes: 
`lai`, `lidf_a`, `hotspot`, `n_struct`, `chl`, `lma`, `cw_rel`, `brown`, 
`psoil`, `ewt`, `cbc`, `prot`,  `ant`, `car`, `tts`, `tto`, `psi`, `type_lidf` 
and `alpha` (note that `type_lidf` and `alpha` are usually set to their default 
value, `type_lidf = 2` and `alpha = 40`).
...
## Produce simulated surface reflectance LUT corresponding to `input_prosail`

The surface reflectance LUT is produced from `input_prosail` with the function 
`generate_lut_surf_refl`.
...
The surface reflectance LUT obtained then corresponds to spectral information 
covering the spectral domain from 400 nm to 2500 nm with 1 nm spectral sampling, 
unless these variables were modified by user. 
This can be the case when adjusting `spec_soil` based on soil reflectance 
extracted from an acquisition. 
In this case, `spec_prospect` and `spec_atm` should be adjusted in order to 
match the sensor characteristics before `train_prosail_inversion` is called.


## Apply sensor spectral response function (SRF) to the surface reflectance LUT
...
`get_srf_sensor` and providing the name of the sensor of interest
as input variable `sensor_name`.

Alternatively, SRF can be computed from `get_srf_sensor` by 
defining a list `spectral_properties` including `wl` which corresponds to the 
central wavelength and `fwhm` which corresponds to the fwhm. 
In this case, the sensor bands are assumed to have a gaussian response.


## Add noise to the sensor surface reflectance LUT
...
Alternatively, multiplicative noise can be applied on surface reflectance by 
setting a value between 0 and 1 for `noise_level` and for each regression model 
(1 is a multiplicative noise corresponding to 100% of the reflectance value). 


## Train SVM regression

Once the input PROSAIL and corresponding sensor surface reflectance LUT with 
added noise are produced, the SVM models can be trained. 
...
prosail_path <- 'hybrid_inversion'
dir.create(path = prosail_path, showWarnings = FALSE,recursive = TRUE)
...
# train model
hybrid_mod <- train_prosail_inversion(parms_to_estimate = parms_to_estimate,
                                      atbd = TRUE, geom_acq = geom_acq, 
                                      srf = srf, selected_bands = selected_bands, 
                                      output_dir = prosail_path)
```
...
- produce surface reflectance LUT
```{r refl_lut_atbd}
...
- Add noise in reflectance
```{r Noise_atbd}
refl_lut_noise <- list()
for (parm in parms_to_estimate) 
  refl_lut_noise[[parm]] <- apply_noise_atbd(refl_lut = refl_lut)
```
...
hybrid_mod <- list()
for (parm in parms_to_estimate)
  hybrid_mod[[parm]] <- prosail_hybrid_train(refl_lut = refl_lut_noise[[parm]],
                                             nb_bagg = 10,
...
The following example applies the regression models to the simulated reflectance 
...
for (parm in parms_to_estimate){
  estimated_bp <- prosail_hybrid_apply(regression_models = hybrid_mod[[parm]],
                                       refl = refl_lut_noise[[parm]])
  mean_estimate[[parm]] <- estimated_bp$mean_estimate
  sd_estimate[[parm]] <- estimated_bp$sd_estimate
}
```
...
for (parm in parms_to_estimate){
  ...
  filename <- file.path(prosail_path, paste(parm,'_atbd.png',sep = ''))
...
```{r, eval=TRUE, echo = FALSE, out.width="100%"}
url <- "https://gitlab.com/jbferet/myshareddata/-/raw/master/prosail_vignette_illustrations/Hybrid_Inversion/hybrid_atbd.png"
knitr::include_graphics(url)
```
...
<center>
  ...
  `albedo` from a simulated dataset corresponding to Sentinel-2 reflectance 
  using PROSAIL-D hybrid inversion based on bagged SVR.
</center> 
<p>&nbsp;</p>
...
## Produce simulated reflectance LUT corresponding to `input_prosail`
...
# define output directory where LUTs will be saved
prosail_path <- 'hybrid_inversion_user'
dir.create(path = prosail_path, showWarnings = FALSE,recursive = TRUE)

# define min and max values for PROSAIL input variables
minval <- data.frame('n_struct' = 1.0, 'chl' = 10, 'car' = 0, 'ant' = 0, 
                     'ewt' =0.001, 'lma' = 0.001, 'brown'=0.01, 'lai' = 0.1, 
                     'soil_brightness' = 0.3, 'lidf_a' = 30, 'hotspot'=0.1, 
                     'tto' = 0, 'tts' = 20, 'psi' = 100)
maxval <- data.frame('n_struct' = 2.0, 'chl' = 60, 'car' = 15, 'ant' = 5, 
                     'ewt' =0.045, 'lma' = 0.040, 'brown'=0.5, 'lai' = 5.0,
                     'soil_brightness' = 3, 'lidf_a' = 70, 'hotspot'=0.5,
                     'tto' = 5, 'tts' = 30, 'psi' = 150)
...
type_distrib <- data.frame('chl' = 'gaussian', 'car' = 'uniform', 
                           ...
                           'n_struct' = 'uniform', 'soil_brightness' = 'uniform', 
                           'lidf_a' = 'uniform', 'lai' = 'gaussian',
                           'hotspot'='uniform', 'tto' = 'uniform', 
                           'tts' = 'uniform', 'psi' = 'uniform')

# define mean and standard deviation for gaussian distributions
mean_var <- data.frame('chl'=45,'lai' = 2.5)
sd_var <- mean_var/2
gaussian_distrib <- list('mean' = mean_var, 'sd' = sd_var)
```

A multiplicative noise can also be defined and applied to simulated reflectance. 
...
options$type_distrib <- type_distrib
options$gaussian_distrib <- gaussian_distrib
...
- produce reflectance LUT
```{r Generate_refl_lut_user}
res <- generate_lut_prosail(SAILversion = '4SAIL',
                            input_prosail = input_prosail,
                            spec_prospect = prosail::spec_prospect_full_range,
                            spec_soil = spec_soil_atbd_v2,
                            spec_atm = spec_atm)
refl_lut_1nm <- res$surf_refl
input_prosail$fcover <- res$fcover
input_prosail$fapar <- res$fapar
...
- Add noise in reflectance and produce SVR model
```{r Noise_user}
refl_lut_noise <- hybrid_mod <- list()
for (parm in parms_to_estimate){
  subsetRefl <- refl_lut[selected_bands[[parm]],]
  refl_lut_noise[[parm]] <- subsetRefl + 
...
The following example applies the regression models to the simulated refl 
produced during the training stage. 
This is of course not recommended, and used only for illustrative purpose.
...
mean_estimate <- sd_estimate <- list()
for (parm in parms_to_estimate){
  hybrid_res <- prosail_hybrid_apply(regression_models = hybrid_mod[[parm]],
                                     refl = refl_lut_noise[[parm]])
  mean_estimate[[parm]] <- hybrid_res$mean_estimate
  sd_estimate[[parm]] <- hybrid_res$sd_estimate
}
```
...
for (parm in parms_to_estimate){
  ...
  ggplot(data = Results, aes(x=estimated, y=measured)) +
    ...
    ylab(paste('Measured', parm)) +
    xlab(paste('Estimated', parm))
  filename <- file.path(prosail_path, paste(parm,'_user.png',sep = ''))
  ggsave(filename, plot = last_plot(), device = 'png', path = NULL,
         scale = 1, width = 5.5, height = 5, units = "in",
         dpi = 600, limitsize = TRUE)
}
...
```{r, eval=TRUE, echo = FALSE, out.width="100%"}
url <- "https://gitlab.com/jbferet/myshareddata/-/raw/master/prosail_vignette_illustrations/Hybrid_Inversion/hybrid_user.png"
knitr::include_graphics(url)
```
<!-- <p float="left"> -->
...
<center>
  ...
  `albedo` from a simulated dataset corresponding to Sentinel-2 reflectance using 
...
PROSAIL (`input_prosail`) parameters and corresponding reflectance LUT (`refl_lut`) as 
