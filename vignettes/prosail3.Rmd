...
The production of a surface reflectance LUT requires a variable `input_prosail` as input. 
...
The surface reflectance LUT obtained then corresponds to spectral information covering the 
...
`get_spectral_response_function` and providing the name of the sensor of interest
as input variable `sensor_name`.

Alternatively, SRF can be computed from `get_spectral_response_function` by 
...
modelSVR <- list()
for (parm in parms_to_estimate)
  modelSVR[[parm]] <- prosail_hybrid_train(refl_lut = refl_lut_noise[[parm]],
...
# define output directory where LUTs will be saved
prosail_path <- 'HybridInversion_User'
dir.create(path = prosail_path, showWarnings = FALSE,recursive = TRUE)

# define min and max values for PROSAIL input variables
minval <- data.frame('n_struct' = 1.0, 'chl' = 10, 'car' = 0, 'ant' = 0, 
                     'ewt' =0.001, 'lma' = 0.001, 'brown'=0.01, 'psoil' = 0.0, 
                     'lidf_a' = 30, 'lai' = 0.1, 'hotspot'=0.1, 'tto' = 0, 
                     'tts' = 20, 'psi' = 100)
maxval <- data.frame('n_struct' = 2.0, 'chl' = 60, 'car' = 15, 'ant' = 5, 
                     'ewt' =0.045, 'lma' = 0.040, 'brown'=0.5, 'psoil' = 1.0, 
                     'lidf_a' = 70, 'lai' = 5.0,'hotspot'=0.5,'tto' = 5,
                     'tts' = 30, 'psi' = 150)
