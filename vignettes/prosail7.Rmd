...
vignette: >
  ...
  %\VignetteIndexEntry{7. Miscellaneous}
...
spec_sensor <- apply_sensor_characteristics(wvl = wvl,
                                            refl = spec_prospect_full_range,
                                            srf = srf)
# apply sensor characteristics to atmospheric properties
spec_atm_sensor <- apply_sensor_characteristics(wvl = wvl,
                                                refl = spec_atm,
                                                srf = srf)
# apply sensor characteristics to soil properties
spec_soil_sensor <- apply_sensor_characteristics(wvl = wvl,
                                                 refl = spec_soil,
                                                 srf = srf)
```
...
# apply sensor characteristics to PROSPECT optical constants
properties_sensor <- prepare_sensor_simulation(spec_prospect = spec_prospect_full_range,
                                               spec_soil = spec_soil,
                                               spec_atm = spec_atm,
                                               srf = srf)
...
... Then surface reflectance, fAPAR and albedo can be computed based on these values:
...
Set `method <- 'nu-svr'` when calling `train_prosail_inversion` to use 
`kernlab::ksvm` with rbf kernel.

Additional methods can be implemented with the support of users. 
