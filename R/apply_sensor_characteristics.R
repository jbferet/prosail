...
apply_sensor_characteristics <- function(wvl, input_refl_table, SRF){
  ...
  for (i in seq_len(nbBands_Sensor)){
    ...
    if (length(indexIN) == length(indexOUT)){
      pond <- matrix(band_values,nrow = 1)%*%as.matrix(input_refl_table[indexOUT,])
      ...
      OutRefl[[i]] <- matrix(0, ncol = ncol(input_refl_table), nrow =1)
    }
  }
  ...
  names(OutRefl) <- colnames(input_refl_table)
  return(OutRefl)
}
