...
parameters of vegetation, such as leaf area index (LAI), chlorophyll content, 
and fractional vegetation cover. 
PROSAIL's ability to simulate bidirectional reflectance factor (BRF) under various 
conditions makes it a valuable tool for agricultural monitoring, ecological 
studies, and climate research.

Here, we introduce `prosail`, an R package aiming at providing multiple versions 
of the model PROSAIL, coupled with the latest versions of PROSPECT distributed 
with the R package `prospect` [@feret2024]. 
...
response function and simulate any sensor. 
It also includes functions aiming at inverting PROSAIL using iterative 
optimization with and without prior information, as well as a module dedicated 
to hybrid inversion applicable to matrices and raster data. 
...
and scattering. 
At leaf scale, the model PROSPECT (leaf optical PROperties SPECtra) is currently 
the most popular physical model for the simulation of leaf optical properties. 
...
machine learning algorithms: RTMs are used to produce a BRF dataset with 
corresponding vegetation properties, then used as training dataset to adjust a 
regression model with a machine learning algorithm. 
Such strategies, commonly referred to as hybrid methods [@verrelst2015], are 
commonly used to process images: simulations are used in place of 
extensive in situ sampling, allowing regression model adjustment on virtually 
any source of optical imagery, while the use of machine learning ensures 
computation efficiency when processing imagery data. 
...
applications. Various softwares currently allow application of hybrid inversion
based on PROSAIL simulations. 

The SNAP toolbox provides an implementation of PROSAIL hybrid inversion based 
combining simulations with an artificial neural network regression model, for 
the estimation of multiple vegetation properties, including LAI, fAPAR, fcover, 
canopy chlorophyll content and canopy water content. 
A set of regression models trained for various geometries of acquisition and the 
spectral response function of Sentinel-2A or Sentinel-2B are included in the 
software, and the most appropriate model selected depending on the Sentinel-2 
acquisition to process. 
The strategy applied for the definition of the sampling of the training dataset, 
including the distribution and co-distribution of the input parameters, is 
described in the Algorithmic Theoretical Background Document (ATBD [@weiss2020])
and cannot be adjusted by users.
Alternative distributions exist, including the ARTMO toolbox [@rivera2014], 
which provides more interactive parameterization of the inversion strategy. 
...
4SAIL2 [@verhoefbach2007], which is a two layers version of SAIL accommodating 
horizontal and vertical heterogeneities.

The R package `prosail` provides a set of functions to help researchers aiming 
at experimenting for the estimation vegetation biophysical properties 
from optical remote sensing using RTM. These functions include : 

- the simulation of BRF for any optical sensor, based on their spectral response 
function

- the simulation of look up tables for sensor BRF and corresponding PROSAIL 
input parameters, with possibility to add noise, and to follow specific input 
parameter distribution

- the application of hybrid inversion on data provided as matrices or rasters

- the application of iterative optimization with possible added prior 
information on data provided as matrices

The package `prosail` does not intend to provide the same computational 
efficiency as softwares such as the Sentinel Application Platform (SNAP) 
[@weiss2020], or a collection of models and methods as comprehensive as those 
provided with the ARTMO box [@rivera2014]. 
It aims at providing a flexible framework to experiment with a simple yet fast 
and efficient hybrid inversion procedure, allowing experimenting on strategies 
for training data sampling, addition of noise, and feature selection over any 
type of optical sensor, and applying regression models easily on imagery data. 
...
Note that PROSAIL are also available in packages written in 
...
[@feret2024] and readers are invited to refer to the documentation of this package for 
a comprehensive description of the leaf chemical and structure parameters 
accounted for by the various versions of the PROSPECT model. 
...
The version `4SAIL2` requires additional parameters: 

- the fraction of LAI corresponding to brown leaves, or a second type of leaves

- a layer dissociation factor between the main leaf layer and the brown leaf 
layer
...
- the optical properties corresponding to the brown vegetation (or the second 
type of leaves). 

SAIL produces a list of output variables corresponding to four top-of-canopy 
reflectance factors. 
Additional parameters required to compute albedo, fraction of absorbed 
photosynthetically active radiation (fAPAR) and Fraction of green Vegetation 
Cover (fcover) are also provided as outputs. 
...
The function `PRO4SAIL` using `4SAIL2` requires different input parameters : 
...
fraction_brown <- 0.5   # fraction of LAI corresponding to brown leaves
...
Users can also define their own sensors, either by providing central wavelength 
and full width half maximum (FWHM) corresponding to each band and assuming 
Gaussian response for each band, or by providing the exact SRF. 
...
SpectralProps = data.frame(wl = wl, fwhm = fwhm)
SensorName <- 'Hyperspectral_Sensor'
SRF_HSI <- prosail::GetRadiometry(SensorName = SensorName,
...
This means that initial conditions influence the output of a local optimization. 
...
error between a reference BRF and BRF simulated with PROSAIL. 
The example code below provides an example of iterative optimization applied to
simulated BRF at native spectral sampling (1 nm) and when using Sentinel-2 SRF. 
...
est_s2 <- Invert_PROSAIL(brfMES = brf_S2, InitialGuess = Init,
                         LowerBound = LB, UpperBound = UB,
...
- the definition of `InitialGuess` variable according to initial assumptions

- the addition of a term corresponding to the probabiliy density function of 
part or all of the parameters to be estimated in the cost function. 

This prior information can 

```r
# define prior information on average leaf angle LIDFa
PriorInfoMean <- list('LIDFa' = 60)
PriorInfoSD <- list('LIDFa' = 10)
WeightPrior <- 0.01

# invert PROSAIL on BRF with 1 nm spectral sampling and probability density 
# function for LIDFa
est_1nm_p <- Invert_PROSAIL(brfMES = brf_1nm$BRF, InitialGuess = Init,
                            LowerBound = LB, UpperBound = UB,
                            SpecPROSPECT_Sensor = SpecPROSPECT_FullRange,
...
est_s2_p <- Invert_PROSAIL(brfMES = brf_S2, InitialGuess = Init,
                           LowerBound = LB, UpperBound = UB,
                           SpecPROSPECT_Sensor = SpecPROSPECT_s2,
...
- The first step consists in using RTM to produce a simulated dataset including 
sensor BRF and corresponding PROSAIL input parameters. 

- The second step consists in training a machine learning regression algorithm 
to estimate a vegetation property or a set of vegetation properties of interest 
based on sensor BRF.

This type of approach can then take advantage of the computational efficiency of 
machine learning algorithms, and leverages RTM to avoid dependency to 
experimental data.
...
The default parameterization produces a BRF LUT compliant with the 
specifications defined in the ATBD for the Biophysical Processor of the Sentinel 
toolbox. 

However, each step of the simulation of a training dataset can be user-defined.
...
training stage. 
The predicted value is then computed based on the mean prediction from a set of 
SVR models.
...
res <- Generate_LUT_PROSAIL(SAILversion = '4SAIL', InputPROSAIL = InputPROSAIL,
...
The resulting models can then be applied on data tables or matrices. 
...
![Biophysical properties estimated with `prosail` vs Biophysical properties 
estimated with SNAP. \label{fig:SNAP_prosail}](compare_SNAP_prosail.png){ width=90% }
...
other softwares such as the Sentinel toolbox. The main advantage is that it 
offers a fully adjustable hybrid inversion framework, including an original 
parsimonious machine learning regression method, allowing fast and efficient 
training for any type of sensor. It is a valuable tool for experimenting on the 
potential of new sensors. 
